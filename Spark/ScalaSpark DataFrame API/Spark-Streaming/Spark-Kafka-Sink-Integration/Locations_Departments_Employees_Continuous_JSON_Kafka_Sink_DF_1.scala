...
object Locations_Departments_Employees_Continuous_JSON_Kafka_Sink_DF_1 {
...
//  define join operation between employees and loc_dept
    ...
    val kafka_df = emp_loc_dept.select(col("department_name").as("key"),
      to_json(struct(col("createdtime"), col("employee_id"), col("location_id"), col("dept_id"),
        ...
        col("hire_dt"), col("salary"))).cast("String").alias("value")
    )
