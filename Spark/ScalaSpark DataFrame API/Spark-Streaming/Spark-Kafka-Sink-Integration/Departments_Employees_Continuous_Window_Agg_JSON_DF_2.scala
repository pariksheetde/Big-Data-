...
object Departments_Employees_Continuous_Window_Agg_JSON_DF_2 {
...
//  calculate the salary of those employees whose salary is greater than the average salary within each department_id
    ...
    val salary_gt_avg_salary = dept_emp_join.join(agg_avg_salary, join_cond, "inner")
      .selectExpr("createdtime", "StartTime", "EndTime", "employee_id", "dept_id", "f_name", "l_name", "department_name",
        "manager_id", "salary", "avg_salary", "hire_dt")
    ...
    val kafka_df = salary_gt_avg_salary.select(col("department_name").alias("key"),
      to_json(struct(col("createdtime"), col("StartTime"), col("EndTime"), col("employee_id"),
        col("dept_id"), col("f_name"), col("l_name"), col("department_name") ,col("manager_id"), col("salary"), col("avg_salary"),
