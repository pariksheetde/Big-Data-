...
object Locations_Departments_Employees_Data_Analysis_2 {
  ...
  def compute() = {
    ...
    val loc_dept_emp_df = loc_dept_df.join(emp_df, loc_dept_df.col("dept_id") === emp_df.col("department_id"))
      .select( col("employee_id"), col("dept_id"), col("loc_id"),
        col("first_name"), col("last_name"), col("hire_date"), col("address"), col("city"), col("state"),
        dayofmonth(col("hire_date")).alias("day"), month(col("hire_date")).alias("month"), year(col("hire_date")).alias("year"))
