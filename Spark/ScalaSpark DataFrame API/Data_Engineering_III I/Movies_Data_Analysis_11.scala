...
object Movies_Data_Analysis_11 {
  println("Movies Data Analysis 11")

  val sparkAppConfig = new SparkConf()

  sparkAppConfig.set("spark.app.name", "Movies Data Analysis 11")
  ...
  def main(args: Array[String]): Unit = {
    ...
    val release_dt_cln = profit_null_df.select(col("Title"), col("Director"), col("Release_date"), col("Major_Genre"))
    ...
    //  calculate number of movies released per year for each director
    println(s"calculate number of movies released per year for each director")
    val title_year_agg = movies_release_dt_df.select(col("Title"), col("Year"), col("Director"))
      .groupBy(col("Year"), col("Director"))
      .agg(
        count(col("Title")).as("Number_of_Movies")
      )
      .where("Director is not null")
      .sort(col("Number_of_Movies").desc)
    title_year_agg.show(false)

    //  calculate number of movies, genre released per year for each director
    println(s"calculate number of movies, genre released per year for each director")
    val title_genre_per_year = movies_release_dt_df.select(col("Title"), col("Director"))
