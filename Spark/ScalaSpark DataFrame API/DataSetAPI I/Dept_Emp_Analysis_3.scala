...
object Dept_Emp_Analysis_3{
  println("Departments - Employees Analysis 3")
  ...
  def compute() = {
    ...
    val dept_emp_join_df = broadcast(Dept_ID_Renamed).join(emp_df, dept_emp_join_expr, "right")
    ...
    println(s"Record Count ${dept_emp_join_df.count()}") // 107
  }
