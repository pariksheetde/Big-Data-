...
games_sel = games.select("game_id", "season", "type", "away_goals", "home_goals",
...
SQL_qry = spark.sql("""
                    ...
                    sum(away_goals) over (partition by substr(season, 1, 4) order by game_id) as sum_home_goals_season,
                    sum(home_goals) over (partition by substr(season, 1, 4) order by game_id) as sum_away_goals_season
                    from games
                    order by season asc
                    """)
